...
void setup()
{
    ...
    pinMode(OPEN_BUTTTON, INPUT_PULLUP);
...
void loop()
{
    ...
    state_door_detected_open = !digitalRead(OPEN_BUTTTON); 
...
void door_init()
{
    ...
    while (!is_door_detected_closed)
    {
        ...
        is_door_detected_closed = !digitalRead(PIN_DOOR_CLOSED);
        state_door_detected_open = !digitalRead(OPEN_BUTTTON); // Inverse de la lecture sur entrée BP bas
        if (is_door_detected_closed)
        {
            Serial.println("Door closed");
            Stop();
